...
function buildUnifiedStatsRecordsFromPayload (payload, isPrivate, dimensionLookup, options: any = {}) {
  const data = payload || {}
  const records = []
 ...
 */
function enrichUnifiedHistoryRowsWithChallengeMetadata (rows, challengeMetadataById, dimensionLookup?: any) {
...
async function getLegacyDistribution (query, fields) {
  ...
  let result: any = { track: query.track, subTrack: query.subTrack, distribution: {} }
