...
function omitMemberAttributes (currentUser, mb) {
  ...
  if (!canManageMember && !hasSensitiveDataRole) {
    // Email is always communication-secure, including when the mutable config
    // object is changed after its startup-time normalization.
    res = _.omit(res, [...config.COMMUNICATION_SECURE_FIELDS, 'email'])
    if (res.phones) {
      delete res.phones
    }
  }
...
async function updateMember (currentUser, handle, query, data) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const includeFields: any = { addresses: true }
...
async function verifyEmail (currentUser, handle, query) {
  ...
  const result = await prisma.member.update({
    where: { userId: member.userId },
    // Loaded relations are not valid scalar update data in Prisma 7.
...
async function uploadPhoto (currentUser, handle, files) {
  ...
  const fileName = handle + '-' + new Date().getTime() + '.' + fileExt
 ...
 */
async function fetchCertificationsAndCourses (userId): Promise<any> {
  try {
